    ...
    if let Some(min) = definition.min_length {
        if value.chars().count() < min {
            errors.push(ValidationError {
                ...
                message: format!(
                    "Field \"{field_name}\" must have at least {min} characters."
                ),
            });
        }
    }
    ...
    if let Some(max) = definition.max_length {
        if value.chars().count() > max {
            errors.push(ValidationError {
                ...
                message: format!(
                    "Field \"{field_name}\" must have at most {max} characters."
                ),
            });
        }
    }
    ...
    if let Some(maximum) = definition.maximum {
        if value > maximum {
            errors.push(ValidationError {
                ...
                code: "MAXIMUM".into(),
                message: format!(
                    "Field \"{field_name}\" must be less than or equal to {maximum}."
                ),
            });
        }
    }
