...
fn benchmark_lookup(page: &PageNode, path: &NodePath) -> u128 {
    let start = Instant::now();

    for _ in 0..ITERATIONS {
        let node = path.get(page);
...
fn benchmark_subtree_with_lookup(
    ...
    for _ in 0..ITERATIONS {
        let node = std::hint::black_box(path.get(page));
...
fn print_result(name: &str, total_ns: u128) {
    ...
    println!(
        ...
        name, total_ms, throughput,
    );

    println!("{:<28} avg: {:>10.6} ms", "", avg_ms,);
}
...
fn main() {
     ...
     *         └── card-3  [2]
     */
    let path = NodePath::from_indexes(vec![1, 1, 2]);

    let node = path.get(&page).expect("benchmark path must exist");

    assert_eq!(node.id, "card-3",);

    let full_result = validate_page_compiled(&page, &compiled);

    let subtree_result = validate_at(&page, &compiled, &path);
    ...
    println!("Page Engine - incremental benchmark");
    ...
    let full = benchmark_full(&page, &compiled);

    print_result("Full page validation", full);

    let lookup = benchmark_lookup(&page, &path);

    print_result("Path lookup only", lookup);

    let subtree = benchmark_subtree(&page, &compiled, &path);

    print_result("Subtree validation", subtree);

    let subtree_lookup = benchmark_subtree_with_lookup(&page, &compiled, &path);

    print_result("Lookup + subtree validation", subtree_lookup);
}
